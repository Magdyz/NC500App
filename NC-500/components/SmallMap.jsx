        ...
        {dayLocations.map((location, index) => {
          return (
            <Marker
              ...
              title={location.name}
              pinColor={
                location.location_id === waypointHighlight ? "aqua" : "red"
              }
              onClick={(e) => mapMarkerFunc(e)}
            ></Marker>
          );
        })}
        {dayStart !== "noDirection" ? (
          <MapViewDirections
            origin={
              dayStart
                ? dayStart
                : `${dayStartRef[dayNum].lat},${dayStartRef[dayNum].long}`
            }
            destination={
              dayEnd
                ? dayEnd
                : `${dayEndRef[dayNum].lat},${dayEndRef[dayNum].long}`
            }
            apikey={apiKey}
            strokeWidth={5}
            waypoints={waypoints}
